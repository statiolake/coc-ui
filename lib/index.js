...
var ListPicker = class {
  ...
  applyFilter() {
    ...
    const filtered = this.source?.interactive ? this.items : this.items.filter(
      (item) => (item.filterText ?? item.label).toLocaleLowerCase().includes(query)
    );
  ...
  async render() {
    ...
    import_coc2.workspace.nvim.call("nvim_buf_clear_namespace", [state.resultsBuffer, -1, 0, -1], true);
    if (page.length) {
      import_coc2.workspace.nvim.call("nvim_buf_add_highlight", [state.resultsBuffer, -1, "CursorLine", this.selected - start, 0, -1], true);
    }
  ...
  async openWindows(name) {
    ...
    const promptWindow = await import_coc2.workspace.nvim.call("nvim_open_win", [promptBuffer, true, { relative: "editor", row, col, width, height: 1, style: "minimal", border: "none", title: ` ${name} `, title_pos: "left" }]);
    const resultsWindow = await import_coc2.workspace.nvim.call("nvim_open_win", [resultsBuffer, false, { relative: "editor", row: row + 1, col, width, height, style: "minimal", border: "none", focusable: false }]);
...
var CocUi = class {
  ...
  async ensureActivityBar(location) {
    ...
    try {
      ...
      this.activityBarCreation = void 0;
    }
  }
  async createActivityBar() {
    const location = "primarySidebar";
    const surface = this.surface(location);
    const editorWindowId = await this.findEditorWindow();
    if (!editorWindowId) return;
    await import_coc3.workspace.nvim.call("win_gotoid", [editorWindowId]);
