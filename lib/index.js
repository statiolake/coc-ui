...
var LAYOUT_WIDTH_RATIO = 0.72;
var LAYOUT_WIDTH_RATIO_WITH_PREVIEW = 0.88;
...
function computePickerLayout(input) {
  ...
  const minPreviewSpan = LAYOUT_MIN_LIST_WIDTH + LAYOUT_PREVIEW_GAP + LAYOUT_MIN_PREVIEW_WIDTH;
  const allowPreview = input.showPreview && input.columns >= minColumns && available >= minPreviewSpan;
  if (!allowPreview) {
    const width = clampWidth(input.columns, LAYOUT_WIDTH_RATIO);
    const col2 = Math.max(0, Math.floor((input.columns - width) / 2));
    return {
      prompt: { row, col: col2, width, height: 1 },
      results: { row: row + 2, col: col2, width, height }
    };
  }
...
function resolveFocusLine(lines, target) {
