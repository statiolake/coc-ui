...
var ListPicker = class {
  ...
  async accept() {
    ...
    if (this.actionMode) {
      const action2 = source?.actions[this.actionSelected];
      const item2 = this.actionItem;
      if (action2 && item2) await this.executeAction(action2, item2);
      return;
    }
  ...
  installCommands() {
    ...
    for (const [key, command] of actionMappings) {
      void import_coc2.workspace.nvim.call("nvim_buf_set_keymap", [state.promptBuffer, "n", key, `<Cmd>CocCommand ${command}<CR>`, { noremap: true, silent: true, nowait: true }]);
    }
  }
};
var activePicker;
